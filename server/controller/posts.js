...
Router.post('/new', auth, upload.single('post_photo'), postValidator, async (req, res) => {
	try {
		if (req.file) req.body.post_photo = '/uploads/' + req.file.filename;
...
Router.put('/edit/:id', auth, upload.single('post_photo'), postValidator, async (req, res) => {
	try {
		const post = await db.Posts.findByPk(req.params.id);
		if (req.file) {
